...
CONNECTION_STRING = "HostName=raspberrypi-edgecomputing.azure-devices.net;DeviceId=Pi;SharedAccessKey=mWhdPSrcGrC79CVDqJ9WBcocP3qNGioDiKxVSF7XZ70="
...
START_POINT = 100
END_POINT = 500
#CLASSESs = ["person" , "bicycle", "car", "motorbike", "aeroplane", "bus", "train", "truck"
...
ANIMAL_CLASSES =[0]
#ANIMAL_CLASSES = [1]
# get it at https://pjreddie.com/darknet/yolo/ or use your custom weight and cfg file.
YOLOV3_CFG = 'yolov3-tiny_custom.cfg'
YOLOV3_WEIGHT = 'yolov3-tiny_custom_last.weights'
...
def detections_yolo3(net, image, confidence_setting, yolo_w, yolo_h, frame_w, frame_h, classes=None):
    ...
    for detection in layer_output:
            ...
            if confidence > confidence_setting and class_id in ANIMAL_CLASSES:
...
def counting_animal(video_input,video_output, client, skip_frame=1):
        ...
        for obj in tmp_list_object:
            ...
            if check:
                ...
                if check_location(box_y, box_height, height):
                    ...
                    iothub_client_telemetry_sample_run(number_animal,client) 
                ...
                if not check_location(box_y, box_height, height):
                    ...
                    if check_new_object and check_start_line(box_y, box_height):
                        ...
                        draw_prediction(CLASSES, colors, frame, new_object['id'], new_object['confidence'],
                                        box_x, box_y, box_width, box_height)                               
        # Put summary text
        cv2.putText(frame, "Number : {:03d}".format(number_animal), (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                   (0, 0, 255), 2)
        # Draw start line
...
def iothub_client_telemetry_sample_run(number,client):
...
if __name__ == '__main__':
    ...
    counting_animal('Penguin7.mp4','output.avi',client)
