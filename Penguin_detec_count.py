...
CONNECTION_STRING = "HostName=IoTHub-18200001.azure-devices.net;DeviceId=PI-IoTHub-0;SharedAccessKey=313fGpfpqUrg+VYXtF+IL/ncg9cMJwsZw1LLJAcs2C4="
...
def detections_yolo3(net, image, confidence_setting, yolo_w, yolo_h, frame_w, frame_h, classes=None):
    ...
    blob = cv2.dnn.blobFromImage(img, 0.00392, (yolo_w, yolo_h),(0,0,0),True, crop=False)
    net.setInput(blob)
...
def counting_animal(url, video_output, client, skip_frame=1):
    colors = np.random.uniform(0, 255, size=(len(CLASSES), 3))

    # Load yolo model
    net = cv2.dnn.readNetFromDarknet(YOLOV3_CFG, YOLOV3_WEIGHT)
    # Define format of input and output
...
if __name__ == '__main__':
    ...
    counting_animal('http://10.1.201.42/cam-mid.jpg', 'Penguin_detect.avi',client)
